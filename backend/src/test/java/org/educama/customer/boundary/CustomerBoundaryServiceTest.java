...
public class CustomerBoundaryServiceTest {
	...
	public void createAddressResource() {
		...
		Page<Customer> page = partnerService.findAllCustomers(pageRequest);
		CustomerListResource customerListResource = assembler.build(page);
		Assert.assertTrue(customerListResource != null);
		Assert.assertEquals(1, customerListResource.getCustomers().size());
	}
